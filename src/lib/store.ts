...
export async function getJobs(filters?: {
  ...
  // ⚡ Bolt: Replace O(n²) array search with O(n) Map lookup for score mapping
  // Impact: Significant reduction in execution time for large job stores
...
export async function getApplications(): Promise<Application[]> {
  ...
  // ⚡ Bolt: Replace O(n²) array search with O(n) Map lookup for joining jobs and resumes
  // Impact: O(1) lookups significantly speed up retrieving applications from local store
  const jobsMap = new Map(store.jobs.map((job) => [job.id, job]));
  const resumesMap = new Map(store.resumes.map((resume) => [resume.id, resume]));
