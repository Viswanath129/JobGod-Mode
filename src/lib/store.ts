...
export async function getJobs(filters?: {
  ...
  // Optimization: Pre-compute score map for O(1) lookups instead of O(N*M)
...
export async function getApplications(): Promise<Application[]> {
  ...
  // Optimization: Pre-compute maps for O(1) lookups instead of O(N*M)
  const jobMap = new Map(store.jobs.map(j => [j.id, j]));
  const resumeMap = new Map(store.resumes.map(r => [r.id, r]));
